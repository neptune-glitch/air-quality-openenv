...
def run_episode(client, difficulty="easy"):
    ...
    print(f"[START] task=air_quality env=custom model=gpt-3.5-turbo difficulty={difficulty}", flush=True)
    ...
    while not done:
        ...
        rewards.append(r)
        print(f"[STEP] step={obs.step} action={action_str} aqi={obs.aqi} reward={r:.4f} done={str(done).lower()} error={info['error'] or 'null'}", flush=True)

    raw_score = env.get_score()
    score = max(0.001, min(0.999, raw_score))
    success = obs_dict["aqi"] < env.task["target_aqi"]
    print(f"[END] success={str(success).lower()} steps={obs.step} rewards={','.join([f'{r:.4f}' for r in rewards])}", flush=True)
    print(f"Final AQI: {obs_dict['aqi']} | Target: {env.task['target_aqi']} | Score: {score:.4f}", flush=True)
